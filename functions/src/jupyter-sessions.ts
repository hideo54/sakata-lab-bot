    ...
    for (const line of lines.split('\n')) {
        ...
        if (cookie) {
            ...
            for (const session of sessions) {
                ...
                const pid = await connection.exec(
                    `ps aux --sort -%mem | grep ${kernelId} | grep -v grep | awk '{print $2}'`,
                    [],
                );
                if (!pid) continue;
                if (pid.split('\n').length > 1) {
                    console.warn(
                        `Multiple PIDs found for kernel ID ${kernelId}.`,
                    );
                }
...
export const notifyAllBigNotebooks = async ({
    host,
    slackApp,
    slackChannel,
}: {
    ...
    try {
        ...
        const bigNotebooks = usageData
            .filter((data) => data.mem >= 10 && data.notebookPath)
...
export const notifyUnusedBigNotebooks = async ({
    slackApp,
    slackChannel,
}: {
    slackApp: App;
    ...
    for (const host of hostNames) {
        try {
            ...
            const unusedBigNotebooks = bigNotebooks.filter((notebook) =>
                dayjs(notebook.lastActivity).isBefore(
